...
sleep 60
for i in `./get_dst.sh airmed` ; do unbuffer python airmid_scrape.py $i $1 $2 &> logs/flyup_$i.log &  done
sleep 60
for i in `./get_dst.sh elal` ; do unbuffer python elal_scrape.py $i $1 $2 &> logs/flyup_$i.log &  done
